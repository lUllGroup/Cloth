...
{
	#region PluginInfo
	[PluginInfo(Name = "SprayNew", Category = "Animation")]
	#endregion PluginInfo
	public class AnimationSprayNewNode : IPluginEvaluate
	{
		#region fields & pins
		[Input("Input")]
			...
			for (int i = 0; i < SpreadMax; i++) {
				...
				if (FBang[i]) {
					for (int j = 0; j < 1; j++) {
						...
						var vel = new Vector3D(FVel[i],FVel[i+1],FVel[+2]);
						vel.z = 0;
						var acc = new Vector3D(FAcc[i], FAcc[i+1], FAcc[i+2]);
